#!/usr/bin/env python3
"""
CORRECTED R-PEAK DETECTION - Fixing Critical Issues
===================================================

This corrected approach fixes all identified critical issues:

🔧 CRITICAL FIXES:
1. Remove data leakage: Only one sample per R-peak (no augmentation overlap)
...
4. Maintain realistic class imbalance (~1% positive samples)
5. Proper evaluation metrics with realistic expectations
6. Clear reporting of train/validation class distributions

Target: Realistic F1-Score evaluation with proper methodology
...
# Removed StratifiedShuffleSplit - using time-based split instead
...
def process_signal_final(file_path):
    ...
    # Previous analysis revealed Column 0 was sample counter, not ECG
...
def create_rpeak_samples_no_leakage(eeg_raw, eeg_norm, rpeak_binary, config):
    ...
    max_negative_ratio = config.get('max_negative_ratio', 50)  # Max 50:1 ratio
...
def train_final_model(train_loader, val_loader, config):
...
def create_detailed_analysis(model, val_loader, device, timestamp):
    """Create detailed analysis with confusion matrix"""
    model.eval()
    all_predictions = []
    all_targets = []
    ...
    plt.hist(predictions[targets == 0], bins=50, alpha=0.7, label='No R-peak', color='blue')
    plt.hist(predictions[targets == 1], bins=50, alpha=0.7, label='R-peak', color='red')
    plt.axvline(x=best_threshold, color='k', linestyle='--', label=f'Threshold ({best_threshold:.3f})')
    ...
    plt.subplot(2, 2, 4)
    # Final metrics
    if len(np.unique(targets)) > 1 and len(np.unique(final_predictions)) > 1:
        precision, recall, f1, _ = precision_recall_fscore_support(targets, final_predictions, average='binary', zero_division=0)
    ...
    plt.tight_layout()
    plt.savefig(f'outputs/plots/final_analysis_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("🔧 ===== CORRECTED R-PEAK DETECTION =====")
    print("Fixed critical issues for realistic evaluation")
    print("=" * 50)
    ...
    # Process signals
    eeg_raw, eeg_norm, rpeak_binary = process_signal_final(file_path)
    ...
    # Train model using train set, validate on test set (no threshold optimization)
    print(f"\n🎓 Training with fixed threshold validation...")
    model, best_f1, train_losses, val_metrics = train_final_model(train_loader, test_loader, config)
    ...
    torch.save(model.state_dict(), f'outputs/models/corrected_rpeak_model_{timestamp}.pth')
    ...
    with open(f'outputs/logs/corrected_results_{timestamp}.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n💾 Model saved to outputs/models/corrected_rpeak_model_{timestamp}.pth")
    print(f"📄 Results saved to outputs/logs/corrected_results_{timestamp}.json")
