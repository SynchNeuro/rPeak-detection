#!/usr/bin/env python3
"""
ENHANCED R-PEAK DETECTION - Improved Performance & Comprehensive Reporting
==========================================================================

Enhanced version of the best performing classification model with:
1. More epochs (50) with early stopping
2. Better learning rate scheduling (ReduceLROnPlateau)
3. Comprehensive visualizations for HTML reporting
4. Detailed training metrics tracking
...
class EnhancedRPeakCNN(nn.Module):
    """Enhanced CNN architecture with improved performance"""
    def __init__(self, input_size=63, input_channels=2):
        super(EnhancedRPeakCNN, self).__init__()
        ...
        # Classification head with improved dropout
...
def process_signal_enhanced(file_path):
    """Enhanced signal processing with better filtering"""
    ...
    # Improved bandpass filter (0.5-40Hz)
...
def train_enhanced_model(train_loader, val_loader, config):
    """Enhanced training with more epochs and better scheduling"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"  🖥️  Using device: {device}")

    model = EnhancedRPeakCNN(input_size=config['window_size'], input_channels=2).to(device)
...
def create_comprehensive_visualizations(model, val_loader, device, train_losses, val_metrics,
                                      eeg_raw, rpeak_locations, config, timestamp):
    ...
    plt.plot(epochs[:len(val_f1_scores)], val_f1_scores, 'g-', linewidth=2, label='Validation F1')
    plt.xlabel('Epoch')
    plt.ylabel('F1-Score')
    ...
    # 4. Get model predictions for detailed analysis
    model.eval()
    all_predictions = []
    all_targets = []
    ...
    for thresh in thresholds:
        pred_bin = (predictions >= thresh).astype(int)
        if len(np.unique(targets)) > 1 and len(np.unique(pred_bin)) > 1:
            p, r, f1, _ = precision_recall_fscore_support(targets, pred_bin, average='binary', zero_division=0)
            precisions.append(p)
            recalls.append(r)
            f1_scores.append(f1)
        else:
    ...
    plt.axvline(x=best_threshold, color='k', linestyle='--', label=f'Optimal ({best_threshold:.3f})')
    ...
    # 9. Sample counts visualization
    plt.subplot(3, 3, 8)
    # Extract sample counts from the data
    pos_samples = np.sum(targets == 1)
    neg_samples = np.sum(targets == 0)
    total_samples = len(targets)

    categories = ['Positive\nSamples', 'Negative\nSamples', 'Total\nSamples']
    counts = [pos_samples, neg_samples, total_samples]
    colors = ['red', 'blue', 'green']

    bars = plt.bar(categories, counts, color=colors, alpha=0.7)
    plt.title('Validation Set Sample Distribution')
    plt.ylabel('Count')
    ...
    # Add percentage for pos/neg
    plt.text(bars[0].get_x() + bars[0].get_width()/2, bars[0].get_height()/2,
            f'{pos_samples/total_samples*100:.1f}%', ha='center', va='center', fontweight='bold', color='white')
    plt.text(bars[1].get_x() + bars[1].get_width()/2, bars[1].get_height()/2,
            f'{neg_samples/total_samples*100:.1f}%', ha='center', va='center', fontweight='bold', color='white')

    plt.tight_layout()

    # Save the comprehensive plot
    os.makedirs('outputs/plots', exist_ok=True)
    plt.savefig(f'outputs/plots/enhanced_analysis_{timestamp}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("🚀 ===== ENHANCED R-PEAK DETECTION =====")
    print("Improved performance with more epochs and better scheduling")
    print("=" * 55)
    ...
    # Process signals
    eeg_raw, eeg_norm, rpeak_binary, rpeak_locations = process_signal_enhanced(file_path)
    ...
    print(f"\n🎓 Training enhanced model with {config['epochs']} epochs...")
    model, best_f1, train_losses, val_metrics = train_enhanced_model(train_loader, test_loader, config)
    ...
    viz_results = create_comprehensive_visualizations(
        model, test_loader, device, train_losses, val_metrics,
        eeg_raw, rpeak_locations, config, timestamp
    )
    ...
    torch.save(model.state_dict(), f'outputs/models/enhanced_rpeak_model_{timestamp}.pth')
    ...
    with open(f'outputs/logs/enhanced_results_{timestamp}.json', 'w') as f:
        json.dump(results, f, indent=2)

    print(f"\n💾 Model saved to outputs/models/enhanced_rpeak_model_{timestamp}.pth")
    print(f"📄 Results saved to outputs/logs/enhanced_results_{timestamp}.json")
    print(f"📊 Visualizations saved to outputs/plots/enhanced_analysis_{timestamp}.png")
